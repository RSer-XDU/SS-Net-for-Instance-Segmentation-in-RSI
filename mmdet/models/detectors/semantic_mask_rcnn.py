...
class Semantic_Mask_RCNN(BaseDetector, RPNTestMixin, BBoxTestMixin,
                       MaskTestMixin):
    def __init__(self,
                 backbone,
                 neck=None,
        ...
        super(Semantic_Mask_RCNN, self).__init__()
    ...
    def init_weights(self, pretrained=None):
        super(Semantic_Mask_RCNN, self).init_weights(pretrained)
    ...
    def forward_dummy(self, img):
        ...
        if self.with_mask:
            ...
            mask_feats = self.mask_roi_extractor(
                x_outs[:self.mask_roi_extractor.num_inputs], mask_rois)
            if self.with_shared_head:
    ...
    def simple_test(self, img, img_meta, proposals=None, rescale=False):
        ...
        # semantic_pred1,_ = torch.max(torch.squeeze(F.softmax(semantic_pred,dim=1))[1:],dim=0)
        # print(semantic_pred1.size())
        # # bsf_show = bsf * semantic_pred2
        # bsf_show = torch.mean(bsf, dim = 1)
        # bsf_show = torch.squeeze(bsf_show)
        # bsf_show = bsf_show.data.cpu().numpy()
        # plt.subplot(122)
