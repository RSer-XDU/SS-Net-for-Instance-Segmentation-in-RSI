...
class MS_Semantic_Mask_RCNN(BaseDetector, RPNTestMixin, BBoxTestMixin,
                       MaskTestMixin):
    def __init__(self,
                 backbone,
                 neck=None,
        ...
        super(MS_Semantic_Mask_RCNN, self).__init__()
    ...
    def init_weights(self, pretrained=None):
        super(MS_Semantic_Mask_RCNN, self).init_weights(pretrained)
    ...
    def forward_train(self,
        ...
        if self.with_mask:
            ...
            s0_mask_pred, s1_mask_pred, s2_mask_pred = self.mask_head(mask_feats)

            s0_mask_targets, s1_mask_targets, s2_mask_targets = self.mask_head.get_target(
                sampling_results, gt_masks, self.train_cfg.rcnn)
            pos_labels = torch.cat(
                [res.pos_gt_labels for res in sampling_results])
            loss_mask = self.mask_head.loss(s0_mask_pred, s1_mask_pred, s2_mask_pred, s0_mask_targets, s1_mask_targets, s2_mask_targets,
                                            pos_labels)
    ...
    def _mask_forward_test(self,
        ...
        if det_bboxes.shape[0] == 0:
            ...
            if combine:
                s0_mask_pred, s1_mask_pred, s2_mask_pred = self.mask_head(mask_feats)
                segm_result = self.mask_head.get_combine_seg_masks(
                    s0_mask_pred, s1_mask_pred, s2_mask_pred, _bboxes, det_labels, self.test_cfg.rcnn, ori_shape,
                    scale_factor, rescale)
            else:
                s0_mask_pred, s1_mask_pred, s2_mask_pred = self.mask_head(mask_feats)
                segm_result = self.mask_head.get_seg_masks(
                    s2_mask_pred, _bboxes, det_labels, self.test_cfg.rcnn, ori_shape,
                    scale_factor, rescale)
        return segm_result
    ...
    def simple_test(self, img, img_meta, proposals=None, rescale=False):
        ...
        #  segmentation prediction and embedded features
        if self.with_semantic:
            _, semantic_feat = self.semantic_head(bsf)
            #
            if self.semantic_fpn_method == 'multiply':
                bsf = bsf * semantic_feat
            elif self.semantic_fpn_method == 'concate':
        ...
        if not self.with_mask:
            ...
            segm_results = self._mask_forward_test(
                outs, img_meta, det_bboxes, det_labels, combine = self.combine_mask, rescale=rescale)
